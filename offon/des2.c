...
int DES(int op, char *key, char *inblock, char *outblock, int len)
{
    ...
    {
        memset(temp, '\0', 8);
        memcpy(temp, inblock + i, len - i < 8 ? len - i : 8);
        op == 1 ? endes(temp, temp) : dedes(temp, temp);
        memcpy(outblock + i, temp, 8);
    }
    return i;
}

#define master_key "vbossmst"
void mydes(int op_type, char *input, char *output)
{
    char buf1[100];
    int l;
    ...
    if(op_type == 1){
        ...
        if(l % 8 > 0)
            l = (l / 8) * 8 + 8;
        DES(1, master_key, input, buf1, l);
        bcd_encode(output, buf1, l*2);

    }else{
        l = strlen(input);
        bcd_decode(buf1, input, l);
        l = l / 2;
        
        DES(0, master_key, buf1, output, l);
    }
}
...
/*initialize s1-s8 arrays,  1 compression value for sinit*/
