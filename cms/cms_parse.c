...
char tablecode[32];
FILE *logfp;
struct WorkOrderInfo   *WorkOrderPtr,*WorkOrderTail,WOlist[4127];
...
int main(int argc,char **argv)
{
    ...
    /* add by kuangwf begin*/
    fprintf(logfp,"MainSvcConf success!\n");
    fflush(logfp);
		/* add by kuangwf end*/
    ...
    for (;;)
    {
        ...
        for (i=0,OrderInfoPtr=OrderInfoHead.next; OrderInfoPtr; OrderInfoPtr=OrderInfoPtr->next,i++)
        {
           ...
           /* fprintf(logfp,"\nservice work：%04d-%s-%s-%d-%s-%s-%s-%s-%s-%s-%s--\n",i,
            ...
            for (; WorkOrderPtr; WorkOrderPtr=WorkOrderPtr->next)
            {
            		if(debug) fprintf(logfp,"MAINKEY===%s:%s|%s|%s|%c|%d|%s|%s|%s|%s\n",cp_key,WorkOrderPtr->cell_code,WorkOrderPtr->hlr_group,WorkOrderPtr->workorder_id,WorkOrderPtr->workorder_type,WorkOrderPtr->workorder_seq,WorkOrderPtr->issend_code,WorkOrderPtr->is_value_map,WorkOrderPtr->is_mul_com,OrderInfoPtr->hlr_group);
printf("+++++++++++ zhr 11 \n");
            	  if (strcmp(WorkOrderPtr->cell_code,IS_HLR) || strcmp(OrderInfoPtr->hlr_group,WorkOrderPtr->hlr_group)==0)
                {
                   ...
                   else if(strcmp(WorkOrderPtr->issend_code,"0") != 0) /*add 20151230*/
                   {
printf("+++++++++++ zhr 22 \n");
                   		...
                   		if(debug) printf("lua_res=[%s]==========\n",lua_res);
                                ...
                                if(debug) fprintf(logfp,"lua_res=[%s]==========\n",lua_res);
                   		if(strcmp(lua_res,"1") == 0)
                   		{
                   				send_flag = 1;
                   		}
                                else if(strcmp(lua_res,"0") != 0 && strlen(lua_res) != 0)/*yaorl add on 20151230 when issend_code returns command_id*/
                                {
                                        send_flag = 1;
                                        strcpy(WorkOrderPtr->workorder_id,lua_res);
                                }
                   }
