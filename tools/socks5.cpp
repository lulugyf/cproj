...
int create_listen_socket(struct sockaddr_in &echoclient, int listen_port) {
    int serversock;
    ...
    echoserver.sin_port = htons(listen_port);       /* server port */
...
bool handle_handshake(int sock, char *buffer) {
    ...
    if(read_size != sizeof(MethodIdentificationPacket) || packet.version != 5){
        printf("ERR: version mismatch %d\n", packet.version);
        return false;
    }

    if(recv_sock(sock, buffer, packet.nmethods) != packet.nmethods){
        printf("number of methods mismatch\n");
        return false;
        }
    ...
    if(send_sock(sock, (const char*)&response, sizeof(MethodSelectionPacket)) != sizeof(MethodSelectionPacket)
        || response.method == METHOD_NOTAVAILABLE)
        return false;
    printf("method: auth=%d\n",  response.method == METHOD_AUTH);
...
bool handle_request(int sock, char *buffer) {
    ...
    switch(header.atyp) {
        case ATYP_IPV4:
        {
            ...
            if(recv_sock(sock, (char*)&req, sizeof(SOCK5IP4RequestBody)) != sizeof(SOCK5IP4RequestBody))
                return false;
            uint32_t ip = ntohl(req.ip_dst);
            string ip_string = int_to_str(ip);
            printf("to ipv4: %s:%d  %08x %08x\n", ip_string.c_str(), req.port);
            client_sock = connect_to_host(ip, ntohs(req.port));
            break;
        }
        case ATYP_DNAME:
            break;
    ...
    printf("connect_to_host return %d\n", client_sock);
    if(client_sock == -1)
        return false;
...
void parse_args(int argc, char *argv[], int *listen_port) {
    if(argc > 1)
...
int main(int argc, char *argv[]) {
    ...
    int listen_port;
    parse_args(argc, argv, &listen_port);
    int listen_sock = create_listen_socket(echoclient, listen_port);
    if(listen_sock == -1) {
