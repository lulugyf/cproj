...
static char				__realpath[1024]="";
static const char*		_realname;

static t_loghndl*		_plh;
static t_logparam		_lp;
static int				_isfirst=1;
static int				_isinit=0;
static char             _uni_accecpt[256] = ""; // 统一流水， 2013-5-2

// 外部调用, 设置全局统一流水, 暂不支持多线程使用不同的流水
void l4c_set_uniaccept(const char *accept){
	if(accept == NULL){
		//printf("====no uni accept!!!\n");
		_uni_accecpt[0] = 0;
		return;
	}
	strncpy(_uni_accecpt, accept, sizeof(_uni_accecpt)-1);
	_uni_accecpt[sizeof(_uni_accecpt)-1] = 0;
	//printf("=====uni accept[%s]\n", _uni_accecpt);
}
...
static const char* get_progname()
{
	...
	if(_realname==0){
		if(GetModuleFileName(NULL, __realpath, 1024)!=0){

		}else if(strlen(_PROCPATH)!=0){
			char procpath[1024];

			sprintf(procpath, "/proc/%d/%s", GETPID(), _PROCPATH);
			if(readlink(procpath, __realpath, 1024)<0){
				strncpy(__realpath, getenv("_"), 1023);
				__realpath[1023]=0;
			}
		}else{
			if(getenv("_")==NULL) return "";
			strncpy(__realpath, getenv("_"), 1023);
			__realpath[1023]=0;
		}
		if(__realpath==NULL) return "";
		s1=strrchr(__realpath, '\\');
		s2=strrchr(__realpath, '/');
...
static int strvalid(const char* word, int level, unsigned len)
{
	...
	for(;i<1024&&i<len;++i){
		...
		if(level>=1&&((word[i]>='A'&&word[i]<='Z')||(word[i]>='a'&&word[i]<='z')||word[i]=='_')) continue;
		return 0;
	}
	return 1;
}
...
static int is_maintag(char* line, const char* main_tag)
{
	...
	if(memcmp(main_tag, line+f, mainlen)==0&&mainlen==t-f+1) return 2;
	else return 1;
}
...
char* l4c_getcfgstr(const char* path, const char* main_tag, const char* sub_tag, char* value, size_t len, const char* def)
{
	...
	while(fgets(line, _MAX_VALUE_LEN-1, fp)!=NULL){
		...
		if(line_len-1>=0&&line[line_len-1]=='\r') line[--line_len]=0;
		if(line_len-1>=0&&line[line_len-1]=='\n') line[--line_len]=0;
		if(line_len-1>=0&&line[line_len-1]=='\r') line[--line_len]=0;
...
static unsigned transtt(char* value)
{
	...
	while((tt=strchr(value, ','))||last_tt){
		...
		lrtrim(last_tt);
		switch(last_tt[0]){
			case 'N':if(strcmp(last_tt, "NEVER")==0) type=type|TT_NEVER; break;
			case 'D':if(strcmp(last_tt, "DALIY"	)==0) type=type|TT_DALIY; break;
			case 'R':if(strcmp(last_tt, "ROLLING")==0) type=type|TT_ROLLING; break;
			case 'I':if(strcmp(last_tt, "INTERVAL")==0) type=type|TT_INTERVAL; break;
		}
		if(tt)
			last_tt=tt+1;
		else
...
static t_fmtfragment* ans_datefmt(char* date_fmt, size_t len)
{
	...
	for(i=0;i<_MAX_VALUE_LEN-1&&date_fmt[i]!=0&&i<len;++i){
		...
		switch(date_fmt[i]){
			case 'y': cur_ft=FT_DATE_YEAR;break;
			case 'M': cur_ft=FT_DATE_MONTH;break;
			case 'd': cur_ft=FT_DATE_DAY;break;
			case 'H': cur_ft=FT_DATE_HOUR;break;
			case 'm': cur_ft=FT_DATE_MINUTE;break;
			case 's': cur_ft=FT_DATE_SECOND;break;
			case 'S': cur_ft=FT_DATE_MILLISECOND;break;
			default:cur_ft=FT_NORMAL;
		}
		if(cur_ft==last_ft){
			++pdf[cur_idx].size;
			continue;
...
static unsigned ans_fmt(char* format, t_fmtfragment** pff1, char* mask)
{
	...
	for(i=0;i<_MAX_VALUE_LEN&&format[i]!=0;++i){
		...
		if(mask){
			if(mask[format[i]]) sh=0;
			else sh=format[i];
		}
		else sh=format[i];

		switch(sh){
			case 'n':pff[cur_idx].ft=FT_NEWLINE;pff[cur_idx].size=1;++cur_idx;break;
			case 'd':{
				char *s1, *s2;
				s1=strchr(format+i, '%');
				s2=strchr(format+i, '}');
			...
			case 'l':pff[cur_idx].ft=FT_LEVEL;pff[cur_idx].size=10;++cur_idx;break;
			case 'r':pff[cur_idx].ft=FT_REALTIME_MS;pff[cur_idx].size=4;++cur_idx;break;
			case 's':pff[cur_idx].ft=FT_REALTIME_S;pff[cur_idx].size=12;++cur_idx;break;
			case 'p':pff[cur_idx].ft=FT_PROCESSID;pff[cur_idx].size=11;++cur_idx;break;
			case 'f':pff[cur_idx].ft=FT_FILENAME;pff[cur_idx].size=1;++cur_idx;break;
			case 'g':pff[cur_idx].ft=FT_FUNCNAME;pff[cur_idx].size=1;++cur_idx;break;
			case 'h':pff[cur_idx].ft=FT_LINENO;pff[cur_idx].size=10;++cur_idx;break;
			case 'm':pff[cur_idx].ft=FT_USER;pff[cur_idx].size=1;++cur_idx;break;
			case 'a':pff[cur_idx].ft=FT_APPNAME;pff[cur_idx].size=1;++cur_idx;break;
			case 'i':pff[cur_idx].ft=FT_IDEN;pff[cur_idx].size=1;++cur_idx;break;
			case '0':pff[cur_idx].ft=FT_V0;pff[cur_idx].size=1;++cur_idx;break;
			case '1':pff[cur_idx].ft=FT_V1;pff[cur_idx].size=1;++cur_idx;break;
			case '2':pff[cur_idx].ft=FT_V2;pff[cur_idx].size=1;++cur_idx;break;
			case '3':pff[cur_idx].ft=FT_V3;pff[cur_idx].size=1;++cur_idx;break;
			case '4':pff[cur_idx].ft=FT_V4;pff[cur_idx].size=1;++cur_idx;break;
			case '5':pff[cur_idx].ft=FT_V5;pff[cur_idx].size=1;++cur_idx;break;
			case '6':pff[cur_idx].ft=FT_V6;pff[cur_idx].size=1;++cur_idx;break;
			case '7':pff[cur_idx].ft=FT_V7;pff[cur_idx].size=1;++cur_idx;break;
			case '8':pff[cur_idx].ft=FT_V8;pff[cur_idx].size=1;++cur_idx;break;
			case '9':pff[cur_idx].ft=FT_V9;pff[cur_idx].size=1;++cur_idx;break;
			case '%':pff[cur_idx].ft=FT_PERCENT;pff[cur_idx].size=1;++cur_idx;break;
			case 't':pff[cur_idx].ft=FT_UNIACCEPT;pff[cur_idx].size=1;++cur_idx;break;
			default:
				if(pff[cur_idx].size==0){
...
static size_t sprint_fmt(time_t now, int tmz[], char* buf,
		size_t len, t_fmtfragment* pff, unsigned num,
		t_logparam* lp, const char* appname,
		const char* iden, const char* var[],
		const char* pam, va_list arg)
{
#define __VARSTUFF(a)		case FT_V ## a : if(var!=NULL){if(var[a]==NULL) v="";else v=var[a];}else v="";sc_size=strlen(v);if(sc_size+total>=len){buf[total]=0;return total;} memcpy(cur, v, sc_size);total+=sc_size;break;
	...
	const char* v;
	char datafmt[]="%04d";
	size_t sc_size=0, total=0;

	for(i=0;i<num;++i){
		char* cur=buf+total;
		if(pff[i].size==0&&pff[i].fmt_fragment==NULL&&pff[i].ft==FT_NORMAL) break;
		if(total+pff[i].size>=len) break;
		switch(pff[i].ft){
			case FT_NORMAL:memcpy(cur, pff[i].fmt_fragment, pff[i].size);total+=pff[i].size;break;
			case FT_NEWLINE:*cur='\n';total+=1;break;
			case FT_DATE:total+=sprint_fmt(now, tmz, cur, len-total, (t_fmtfragment*)pff[i].fmt_fragment, _MAX_DATE_FRAGMENT_NUM, lp, appname, iden, var, pam, arg);break;
			case FT_DATE_YEAR:;
			case FT_DATE_MONTH:;
			case FT_DATE_DAY:;
			case FT_DATE_HOUR:;
			case FT_DATE_MINUTE:;
			case FT_DATE_SECOND:;
			case FT_DATE_MILLISECOND:datafmt[2]=(pff[i].size%10)+'0';sprintf(sc, datafmt, (tmz[pff[i].ft-FT_DATE_YEAR])%(_pow(10,(pff[i].size))));sc_size=strlen(sc);memcpy(cur, sc, sc_size);total+=sc_size;break;
			case FT_LINENO: sprintf(sc, "%d", lp->line_no);sc_size=strlen(sc);memcpy(cur, sc, sc_size);total+=sc_size;break;
			case FT_FUNCNAME: sc_size=strlen(lp->func_name);if(sc_size+total>=len){buf[total]=0;return total;} memcpy(cur, lp->func_name, sc_size);total+=sc_size;break;
			...
			case FT_FILENAME: sc_size=strlen(lp->file_name);if(sc_size+total>=len){buf[total]=0;return total;} memcpy(cur, lp->file_name, sc_size);total+=sc_size;break;
			case FT_APPNAME: sc_size=strlen(appname);if(sc_size+total>=len){buf[total]=0;return total;} memcpy(cur, appname, sc_size);total+=sc_size;break;
			case FT_PROCESSID: sprintf(sc, "%d", GETPID());sc_size=strlen(sc);memcpy(cur, sc, sc_size);total+=sc_size;break;
			case FT_LEVEL :
				if(lp->ll<_LNONE&&lp->ll>=0){
					sc_size=strlen(_level_name[lp->ll]);memcpy(cur, _level_name[lp->ll], sc_size);total+=sc_size;break;
				}
				else{
					sprintf(sc, "LEVEL[%d]", lp->ll);sc_size=strlen(sc);memcpy(cur, sc, sc_size);total+=sc_size;break;
				}
			case FT_REALTIME_S:
				sprintf(sc, "%011d", (int)now);sc_size=strlen(sc);memcpy(cur, sc, sc_size);total+=sc_size;break;
			case FT_REALTIME_MS:
				sprintf(sc, "%03d", tmz[6]);sc_size=strlen(sc);memcpy(cur, sc, sc_size);total+=sc_size;break;
			case FT_USER: sc_size=vsnprintf(cur, len-total, pam, arg);if(sc_size+total>=len){buf[len-1]=0;return len-1;} total+=sc_size;break;
			case FT_IDEN: sc_size=strlen(iden);if(sc_size+total>=len){buf[total]=0;return total;} memcpy(cur, iden, sc_size);total+=sc_size;break;
			case FT_PERCENT: *cur='%';total+=1;break;
			case FT_UNIACCEPT: sc_size=strlen(_uni_accecpt);if(sc_size+total>=len){buf[total]=0;return total;} memcpy(cur, _uni_accecpt, sc_size);total+=sc_size;break;
			__VARSTUFF(0)
			__VARSTUFF(1)
			__VARSTUFF(2)
			__VARSTUFF(3)
...
static void openlogfile(t_log* pl, ...)
{
	...
	if(pl->fp){
		int need_close=0;
		now=time(NULL);
		if(pl->lc.output_type!=OT_FILE) return;
	...
	if(pl->lc.path_len>=pl->lc.max_buf-1) return;
	...
	if(pl->lc.open_clear) open_type[1]=0;
...
{
	switch(errcode){
		case LOGERR_OUTPUTTYPE	:return "[LOGERR_OUTPUTTYPE]输出类型不正确";
		case LOGERR_MALLOC		:return "[LOGERR_MALLOC]内存分配失败";
		case LOGERR_ANS			:return "[LOGERR_ANS]日志格式解析错误";
		case LOGERR_LOGTOOLONG	:return "[LOGERR_LOGTOOLONG]日志内容过长";
		case LOGERR_OPENFILE	:return "[LOGERR_OPENFILE]日志文件打开失败";
		case LOGERR_LOADDEF		:return "[LOGERR_LOADDEF]加载默认配置失败";
		case LOGERR_HANDLE		:return "[LOGERR_LOADDEF]日志句柄无效";
		case 0					:return "正确";
		default: {
			return "未知";
		}
...
void l4c_close(t_loghndl* plh)
{
	...
	if(pl->fp!=NULL&&pl->lc.output_type==OT_FILE) fclose(pl->fp);
	if(pl->buf) free(pl->buf);
	if(pl->lc.need_lock) pthread_mutex_destroy(&pl->lock);
	if(pl->file_nameparam) free(pl->file_nameparam);
	if(pl->filename_fmt){
...
int l4c_open(t_loghndl* plh, const char* cfgpath, const char* main_tag, const char* fileparam, const char* iden)
{
	...
	switch(value[strlen(value)-1]){
		case 'L' :if(strcmp(value, "NULL"		)==0) pl->lc.output_type=OT_NULL; break;
		case 'T' :if(strcmp(value, "STDOUT"		)==0) pl->lc.output_type=OT_STDOUT; break;
		case 'R' :if(strcmp(value, "STDERR"		)==0) pl->lc.output_type=OT_STDERR; break;
		case 'E' :if(strcmp(value, "FILE"		)==0) pl->lc.output_type=OT_FILE; break;
		case 'G' :if(strcmp(value, "SYSLOG"		)==0) pl->lc.output_type=OT_SYSLOG; break;
		default:;
	}
	if(pl->lc.output_type==-1) return LOGERR_OUTPUTTYPE;
	...
	switch(value[0]){
		case 'A' :if(strcmp(value, "ALL"		)==0) pl->lc.lowest_level=_LALL; break;
		case 'T' :if(strcmp(value, "TRACE"		)==0) pl->lc.lowest_level=_LTRACE; break;
		case 'D' :if(strcmp(value, "DEBUG"		)==0) pl->lc.lowest_level=_LDEBUG; break;
		case 'I' :if(strcmp(value, "INFO"		)==0) pl->lc.lowest_level=_LINFO; break;
		case 'W' :if(strcmp(value, "WARN"		)==0) pl->lc.lowest_level=_LWARN; break;
		case 'E' :if(strcmp(value, "ERROR"		)==0) pl->lc.lowest_level=_LERROR; break;
		case 'F' :if(strcmp(value, "FATAL"		)==0) pl->lc.lowest_level=_LFATAL; break;
		case 'B' :if(strcmp(value, "BULLETIN"	)==0) pl->lc.lowest_level=_LBULLETIN; break;
		case 'N' :if(strcmp(value, "NONE"		)==0) pl->lc.lowest_level=_LNONE; break;
		default:;
	}

	if(pl->lc.lowest_level==(e_loglevel)-1) return LOGERR_OUTPUTTYPE;
	...
	pl->lc.flush_type=-1;
	switch(value[0]){
		case 'E' :if(strcmp(value, "EVERY"		)==0) pl->lc.flush_type=HT_EVERY; break;
		case 'L' :if(strcmp(value, "LINE"		)==0) pl->lc.flush_type=HT_LINE; break;
		case 'C' :if(strcmp(value, "CLOSE"		)==0) pl->lc.flush_type=HT_CLOSE; break;
		default:;
	}
	if(pl->lc.flush_type==-1) pl->lc.flush_type=HT_CLOSE;
	...
	if(!pl->lc.muti_thread) pl->lc.need_lock=0;
	...
	if(pl->iden==NULL) return LOGERR_MALLOC;
...
static t_loghndl* defhandle()
{
	...
	if(_isfirst){
		...
		pl=(t_log*)plh;
		if(l4c_open(plh, "l4c.cfg", ".DEFAULT", "", "")!=0){
...
static int vlogr(t_logparam lp, t_loghndl* plh, const char* var[], const char* fmt, va_list vl)
{
	...
	if(pl->lc.output_type==OT_NULL||lp.ll<pl->lc.lowest_level) return 0;

	if(pl->lc.need_lock)
		pthread_mutex_lock(&pl->lock);
	if(pl->lc.output_type==OT_STDOUT) pl->fp=stdout;
...
{
	if(plh==NULL) return LOGERR_HANDLE;
...
int l4c_vlogdef(const char* fmt, va_list vl)
{
	...
	if(plh==NULL) return LOGERR_LOADDEF;
	pl=(t_log*)plh;
	if(pl->lc.muti_thread)
...
void tmp_set(int ll, int line_no, const char* func_name, const char* file_name)
{
	...
	if(_isinit&&pl->lc.muti_thread&&pla==NULL){
...
t_logparam l4c_lvget(t_loghndl* plh)
{
	...
	if(plh==NULL){
		plh=defhandle();
		if(plh==NULL) return lp;
		pl=(t_log*)plh;
	}
	lp.ll=pl->lc.lowest_level;
	return lp;
}
...
{
	if(lp1.ll==lp2.ll) return 0;
	else if(lp1.ll>lp2.ll) return 1;
	else return -1;
}
