...
#ifndef CLK_TCK
#ifndef VMS
#define LOCAL_HZ	60.0
#else /* VMS */
#define LOCAL_HZ	100.0
#endif
#else /* CLK_TCK */
#define LOCAL_HZ ((double)CLK_TCK)
#endif
...
int s;
	...
	if (s == START)
		...
		{
		times(&tend);
		ret=((double)(tend.tms_utime-tstart.tms_utime))/LOCAL_HZ;
...
#define COUNT(d) (count)
...
#ifdef SIGALRM
	printf("Doing docrypt for 10 seconds\n");
	alarm(10);
#else
	printf("Doing docrypt %ld times\n",cd);
#endif
	Time_F(START);
	...
	printf("docrypt         per sec = %12.2f (%5.1fuS)\n",d,1.0e6/d);
	}
