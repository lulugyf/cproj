...
// for json-datatype support  add 201610 json  begin
...
// for json-datatype support  add 201610 json end
...
static void* busiwork1(void* v1, db_connect1* pdb, bool isLong)
{
        TCtrlInfoEx* ctrlInfo=(TCtrlInfoEx*)v1;
        ...
        const char *psUtypeFunc, TUtypeFuncPtr ptUtypeFuncPtr, TDoMerge ptDoMerge, int iConnType)
{
  ...
  //2011/10/28 17:49:40
        ...
        try
        {
        	...
        	/*otl_connect db1;
        	db1.rlogon("DBACCADM/DBACCADM@kfbossb");
        	db1.logoff();
        	printf("--------for direct test db connect only----------------------1\n"); */
                dbFactory.doBegin();//连接主数据库
                CtrlInfo.func=ptUtypeFuncPtr;
                L4C_TRACE("connect db success");
                ibuf=(char *)transb->data;
                errlog_begin(psServiceName, ibuf);
                	/* add  20170505 json  begin */
				if (checkChr(ibuf, '<'))
            ...
            if (inParam.get() == NULL)
                throw(uException(U_ERR_MORE_TYPE,"input json string format invalid."));
								...
								/* add  20170505 json  end */
                //inParam = uXml2Utype(ibuf);
                
                //端到端监控
                CtrlInfo.init1(transb->name, psUtypeFunc, inParam);
...
finally:
        {
                if(!retCode){
                        try{
                                ...
                                errid = bindRetBuff(iRuleErrFlag, &CtrlInfo, outParam.release(), retCode, retMsg,sPromptMsg, obuf, obufLen, outName, isXml);
                                if(sPromptMsg!="")
                                ...
                                errid = bindRetBuff(iRuleErrFlag, &CtrlInfo,NULL,retCode,retMsg,sPromptMsg,obuf,obufLen,NULL, isXml);
                        }
                }
                else{
                        L4C_ERROR("APPERR"_LOG_FMT,retCode,retMsg);
                        errid = bindRetBuff(iRuleErrFlag, &CtrlInfo, tup.release(), retCode,retMsg, sPromptMsg, obuf,obufLen,NULL, isXml);
