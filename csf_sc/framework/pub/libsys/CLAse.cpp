...
#define GETuint32(p) RJ_SWAP(*((ub4 *)(p)))
#define PUTuint32(ct, st) { *((ub4 *)(ct)) = RJ_SWAP((st)); }

#else

#define GETuint32(pt) (((ub4)(pt)[0] << 24) ^ ((ub4)(pt)[1] << 16)\
	^ ((ub4)(pt)[2] <<  8) ^ ((ub4)(pt)[3]))
#define PUTuint32(ct, st) { (ct)[0] = (ub1)((st) >> 24); (ct)[1]\
	= (ub1)((st) >> 16); (ct)[2] = (ub1)((st) >>  8); (ct)[3] = (ub1)(st); }
...
#define AES_BAD_DATA  -1	/* If bad data discovered during decoding */
	...
	typedef struct {
		...
		ub4   rk[4*(AES_MAXNR + 1)];	/* key schedule */
	} KEYINSTANCE;
	...
	*/
	...
	static int my_aes_create_key(KEYINSTANCE *aes_key,
		...
		int key_length)
	{
		ub1 rkey[AES_KEY_LENGTH/8];	 /* The real key to be used for encryption */
		ub1 *rkey_end=rkey+AES_KEY_LENGTH/8; /* Real key boundary */
		ub1 *ptr;			/* Start of the real key*/
		const char *sptr;			/* Start of the working key */
		const char *key_end=key+key_length;	/* Working key boundary*/

		memset((char*) rkey,0, AES_KEY_LENGTH/8);      /* Set initial key  */
...
#define AES_USE_KEY_BYTES (AES_USE_KEY_BITS/8)
	...
	*/
	...
	{
		KEYINSTANCE aes_key;
		ub1 block[AES_BLOCK_SIZE];	/* 128 bit block used for padding */
		int rc;			/* result codes */
		...
		{
			rijndaelEncrypt(aes_key.rk, aes_key.nr, (const ub1*) source,
				(ub1*) dest);
		...
		rijndaelEncrypt(aes_key.rk, aes_key.nr, block, (ub1*) dest);
	...
	*/
		...
		const char *key, int key_length)
	{
		...
		{
			rijndaelDecrypt(aes_key.rk, aes_key.nr, (const ub1*) source,
				(ub1*) dest);
			source+= AES_BLOCK_SIZE;
			dest+= AES_BLOCK_SIZE;
		}

		rijndaelDecrypt(aes_key.rk, aes_key.nr, (const ub1*) source, block);
		/* Use last char in the block as size */
		pad_len = (uint) (ub1) block[AES_BLOCK_SIZE-1];
