...
{"ROOT": {
        "HEADER": {
            ...
            "DB_ID": "",
            "ENV_ID": "1",
            "CONTACT_ID": "412013120310345678901234561", 
            "CHANNEL_ID": "0702", 
            "USERNAME": "zlj",
            "PASSWORD": "zlj@201508030947123",   //此处展示为明文 ，具体由客户端加密后以密文传参，该字段为原密码加上@201508030947123 后再整体加密。
            "ENDUSRLOGINID": "",
            "ENDUSRIP": "10.95.236.85",
            "ROUTING": {
                "ROUTE_KEY":"10",
                "ROUTE_VALUE": "${phone}"
            }
                         ...
                         int call_type, int conn_timeout, int recv_timeout)
{
    ...
    uAutoPtr   uptr_tmpheader(uInit(1));
    uAutoPtr   uptr_tmprouting(uInit(1));
    uAutoPtr   uptr_header(uInit(1));
    if( NULL==ptr_src_header)
    {
        /* 21:25 2013年8月2日 与 wangxd 等同事确认 如果从框架中获取不到
           ESB 需要的 HEAD 信息 那么自动从 crm.cfg 配置中读取默认的
           CALL_ESB_API.POOL_ID 和 CALL_ESB_API.CHANNEL_ID.
           同时要求 行业部订单的程序必须传入 route_key 和 route_value
           用于 ESB 进行路由;
        */
        /* 代码注释掉 如果获取到空值不再抛出异常;
        memset(err_msg, 0, sizeof(errmsg_len));
        ...
        */
        if(NULL == route_key || NULL == route_value)
        {
            memset(err_msg, 0, sizeof(errmsg_len));
            snprintf(err_msg, errmsg_len-1, "%s:%d 获取原始服务的包头信息为NULL 需要读取CALL_ESB_API.POOL_ID和CALL_ESB_API.CHANNEL_ID 作为默认值 此时route_key和route_value为必填参数 请核查这两个参数信息", __FILE__,__LINE__);
            throw( appException(555100001,  err_msg) );
        }
        utAddStr(uptr_tmprouting,"ROUTE_KEY",route_key);
        utAddStr(uptr_tmprouting,"ROUTE_VALUE",route_value);
        utAddStruct(uptr_tmpheader, "ROUTING", uptr_tmprouting.release());
        
        const char * default_pool_id = cfgGetStr("CALL_ESB_API", "POOL_ID");
        if( NULL == default_pool_id || strlen(default_pool_id)==0)
        {
            memset(err_msg, 0, sizeof(errmsg_len));
            snprintf(err_msg, errmsg_len-1, "%s:%d 获取 crm.cfg 中 section为[CALL_ESB_API]的配置:POOL_ID 失败   此配置为后台订单程序在无法获取ESB需要的HEADER节点中的POOL_ID的默认值", __FILE__,__LINE__);
            throw( appException(555100009,  err_msg) );
        }
        utAddStr(uptr_tmpheader,"POOL_ID",default_pool_id);
        uptr_header = uptr_tmpheader;  /* test mm*/
    }
    else
    {//ptr_src_header not NULL 
        uAutoPtr uptr_ttmmpp(utClone(ptr_src_header));
        uptr_header = uptr_ttmmpp;
    }
    ...
    if(route_key != NULL)
    {
        ...
        {
            printf("%s:%d 获取服务入参中的 HEADER.ROUTING 失败:%s\n", __FILE__, __LINE__,appErr.what());
            throw;
        }
	...
	/// guanyf 2015-8-11 添加， ESB增加密码校验：
	const char *username = cfgGetStr("CALL_ESB_API", "USERNAME");
	if(username != NULL && username[0] != 0){
		pos_name = utFind(uptr_header, "USERNAME");
		if(pos_name == UPOS_END)
			utAddStr(uptr_header, "USERNAME", username);
		else
			utSetStr(uptr_header, pos_name, username); //已有则替换
	}
	const char *password = cfgGetStr("CALL_ESB_API", "PASSWORD");
	if(password != NULL && password[0] != 0){
		pos_name = utFind(uptr_header, "PASSWORD");
		if(pos_name == UPOS_END)
			utAddStr(uptr_header, "PASSWORD", password);
		else
			utSetStr(uptr_header, pos_name, password);
	}
 /*   
    const char *channelid = cfgGetStr("CALL_ESB_API", "CHANNEL_ID");
	if(channelid != NULL && channelid[0] != 0){
		pos_name = utFind(uptr_header, "CHANNEL_ID");
		if(pos_name == UPOS_END)
			utAddStr(uptr_header, "CHANNEL_ID", channelid);
		else
			utSetStr(uptr_header, pos_name, channelid);
	} */
