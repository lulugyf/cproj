...
#define L4C_OTL_INFO \
	...
	buf<<"<OTLINFO_SQL>"<<this->stm_text<<"  <OTLINFO_BINDV>";\
  for(int z=0;z<this->vl_len;++z){\
	  int type=this->vl[z]->get_ftype();\
	  buf<<" "<<this->vl[z]->get_name_pos()<<"."<<this->vl[z]->get_name()+1<<"(";\
	if(type==1) buf<<"STRING):"<<((char*)this->vl[z]->get_var_struct().val(0, 0));else if (type==4) buf<<"INT):"<<*((int*)this->vl[z]->get_var_struct().val(0, 0)); else if (type==7) buf<<"LONG):"<<*((long*)this->vl[z]->get_var_struct().val(0, 0));else {buf<<"UNKNOW)";continue;}\
  }if(strcmp(getenv("OTL_INFO")==NULL?"":getenv("OTL_INFO"), "on")==0) L4C_INFO("%s", buf.str().c_str());

class ATOIC{
public:
	static int SPEC_ATOI(const char*s)
	{
		int iRet=0;
		char tmp[32];
		char *t=tmp;
