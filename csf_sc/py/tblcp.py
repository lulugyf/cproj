...
def initlogger(lname, dccfg):
    ...
    fh = logging.handlers.TimedRotatingFileHandler("/crmpdpp/hcai/Eai/plugin/m/ah/appintegration/dbsyn/tblcp/log/tc_%s.log"%lname, when=dccfg.get("logging", "filerotate"))
...
class TableCopy:
    ...
    def loadcfg(self):
        ...
        self.cfg.read("/crmpdpp/hcai/Eai/plugin/m/ah/appintegration/dbsyn/tblcp/etc_incr/"+self.copyname+".conf")
    ...
    def conndb(self, dbname):
        ...
        if tp == "oracle":
             ...
             #need modify
             #decrypt_pro = "/crmpdpp/hcai/Eai/plugin/m/ah/appintegration/dbsyn/bin/BOSSPasswd"
             #print(decrypt_pro)
             #get_dbinfo_srv = '%s %s %s' % (decrypt_pro, dbname, "DBSERV")
             #get_dbinfo_user = '%s %s %s' % (decrypt_pro,dbname, "DBUSER")
             #get_dbinfo_pwd = '%s %s %s' % (decrypt_pro, dbname, "DBPASSWD") 
             #print(get_dbinfo_srv)
             #print(get_dbinfo_user) 
             #print(get_dbinfo_pwd)  		    
